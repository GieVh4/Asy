...
cl = LINETCR.LINE()
cl.login(token='Er1KS95sK8UOaCiaM9Y2.cgM2mbGybJNV6ZLunKEEaG.13UrwlokANEWG2KsqjkuJjU7Fgk1il37ue1uEhIMEUE=')
cl.loginResult()

ki = LINETCR.LINE()
ki.login(token='ErK9lCQsV3vO7E8g6cg1.GGQgfQP7oMPQBTGwA5OiOq.QLd/jseiwGaoXUtMpzbykkdBq5+Zo9Pm23LEfOauJyA=')
ki.loginResult()

kk = LINETCR.LINE()
kk.login(token='EreaOr3aPESiw52eyWf7.DlqRyKJ45By6qR51/9HR5W./3Ye7/O7/fV/aFh2ER5Rkd507bsQyr5LTb6zNJZAbw8=')
kk.loginResult()

ks = LINETCR.LINE()
ks.login(token='Er54qyC1SA8qsx1RwUhf.XZz8P6CsX90xK88xL2AVVW.kH/GBL+7RSUNFwulSeaDuK6KGfBuPh04tNsmuXbnfhE=')
ks.loginResult()

kc = LINETCR.LINE()
kc.login(token='ErxhYUL66OMTb1gbYVAb.0cO2i9McKyZSokcFO8Tp/W.QMYgZzh/jmvGPfiVgRq7RBEl7OcvvN9g7OWG/hJxlzI=')
kc.loginResult()

ka = LINETCR.LINE()
ka.login(token='Erwv7LVIK0W4GCoj6pU6.DOIL1aVKKSMDjRg43snenG.ulODQ2s9X9WCKcvSGIjZL3WpEGdj/47PW28u26TpZYk=')
ka.loginResult()

print "login success"
reload(sys)
sys.setdefaultencoding('utf-8')
helpMessage ="""㊙ GYE_VHA_BOTH ㊗      
🔥 GYE_VHA_BOTH 🔥
🔥 Id]: ~÷~
🔥 Mid]: ~÷~
🔥 All mid]:
...
admin = ["uc38e086fd9e30cb21396c99eb1e06102"]
...
admins = ["uc38e086fd9e30cb21396c99eb1e06102"]
Rx5 = ["u820a8616aa7b3a6a6a6ed8166b9a08a1"]
Rx4 = ["u74b42b6248f8e5b63a42be75fbd86be7"]
Rx3 = ["uc7367ca7a712c7dd1793cf179766afbf"]
Rx2 = ["ud02a3623d982918cec38a09fa3cb4ffb"]
Rx1 = ["u85cdb72bc0141fd9473ef2df11281260"]
...
wait = {
    ...
    'timeline':False,
    'autoAdd':False,
    'message':"㊗ Thanks Forr Add Me 🉐 Bye.GyeVha",
    "lang":"JP",
    "comment":"Auto like 👉GyeVha👈",
    "likeOn":False,
    "commentOn":False,
    "commentBlack":{},
    "wblack":False,
...
def bot(op):
    try:
        ...
        if op.type == 26:
            ...
            if msg.toType == 0:
                msg.to = msg.from_
                if msg.from_ == "uc38e086fd9e30cb21396c99eb1e06102":
                    if "join:" in msg.text:
        ...
        if op.type == 25:
            ...
            if msg.contentType == 13:
                ...
                if wait["commentOn"] == False:
                    ...
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"🔥Comentar Diaktifkan")          
            #elif "gurl" == msg.text:
            ...
            #---------------FUNGSI RATAIN GRUP TANPA KICK SESAMA BOT/Admin/Bots----------#
              ...
              if msg.from_ in Bots:
                if msg.toType == 2:
                    ...
                    ki.sendText(msg.to,"Nah Loh Permisi Ye")
                    kk.sendText(msg.to,"Numpanh Bersih Bersih")
                    kc.sendText(msg.to,"Rata Gak Rata Bodo Amat")
                    ks.sendText(msg.to,"Sikat Semampunya Kawan")
                    msg.contentType = 13
                    msg.contentMetadata = {'mid': mid}
                    ks.sendMessage(msg)
                    targets = []
                        ...
                        for target in targets:
                          if target not in Bots:
                            try:
                                ...
                                kc.sendText(msg.to,"Minggir Dikit Naj.. !!")
                                ka.sendText(msg.to,"Bom Bom Bom.. 🚷")
#-----------------------------------------------------------                          
            elif "Kick" in msg.text:
				...
				if msg.from_ in OWN:
					...
					for h in gs.members:
						if _name in h.displayName:
							targets.append(h.mid)
					if targets == []:
						sendMessage(msg.to,"Rata Gak Rata Masa Bodo")
						pass
					else:
						for target in targets:
							try:
...
#-----------------------------------------------------------
				...
				if msg.to in protection:
					cl.sendText(msg.to,"🔥 Semua Protect Do Aktifkan")
				else:
...
#-----------------------------------------------------------
                ...
                for g in gs.members:
                    if _nametarget == g.displayName:
                       ...
                       kk.sendText(g.mid,"Spam  😂")
                       ks.sendText(g.mid,"Spam  😂")
                       kc.sendText(g.mid,"GyeVha 😂")
                       ka.sendText(g.mid,"GyeVha😂")
                       ks.sendText(g.mid,"GyeVha😂")
                       ks.sendText(g.mid,"Spam  😂")
                       kc.sendText(g.mid,"Spam  😂")
                       ka.sendText(g.mid,"Spam  😂")
                       cl.sendText(g.mid,"Spam  😂")
                       ...
                       kc.sendText(g.mid,"GyeVha😂")
                       ka.sendText(g.mid,"GyeVha😂")
                       cl.sendText(g.mid,"GyeVha😂")
                       ki.sendText(g.mid,"GyeVha😂")
                       kk.sendText(g.mid,"GyeVha😂")
                       ks.sendText(g.mid,"GyeVha😂")
                       cl.sendText(msg.to, "GyeVha😂")
                       print "Done spam" 
#-----------------------------------------------------------
